...
@section('styles')
    ...
    .side-panel, .copy-panel { border-radius: 8px; background: #fff; box-shadow: var(--shadow); }
    .side-panel { position: sticky; top: 94px; padding: 26px; }
    .side-panel h2, .copy-panel h2 { margin: 0 0 14px; color: var(--ink); font-size: 24px; line-height: 1.25; }
    ...
    @media (max-width: 980px) { .detail-hero__grid, .detail-layout { grid-template-columns: 1fr; } .side-panel { position: static; } }
@endsection

@section('content')
    <section class="detail-hero">
        <div class="container detail-hero__grid">
            <div><p class="eyebrow">{{ $service['label'] }}</p><h1>{{ $service['title'] }}</h1><p>{{ $service['summary'] }}</p><a class="button" href="{{ url('/intake-v2') }}">Book a consultation</a></div>
            <div class="detail-image"><img src="{{ asset($service['image']) }}" alt="{{ $service['label'] }}"></div>
        </div>
    </section>
    <section class="section">
        <div class="container detail-layout">
            <aside class="side-panel"><h2>Service pathway</h2><div class="registration">{{ $service['registration'] }}</div><p>Our team can help you understand fit, intake details, and the best next step for this support.</p><a class="button" href="{{ url('/contact-v2') }}">Ask about this service</a></aside>
            <article class="copy-panel">
                @if(!empty($service['heading']))<h2>{{ $service['heading'] }}</h2>@endif
                @foreach($service['intro'] as $paragraph)<p>{{ $paragraph }}</p>@endforeach
                <h2>{{ $service['section_heading'] }}</h2><p>{{ $service['section_intro'] }}</p>
                @if(!empty($service['items_heading']))<h2>{{ $service['items_heading'] }}</h2>@endif
                @if(!empty($service['items_intro']))<p>{{ $service['items_intro'] }}</p>@endif
                <ul class="support-list">@foreach($service['items'] as $item)<li><strong>{{ $item['title'] }}:</strong> {{ $item['text'] }}</li>@endforeach</ul>
                <div class="detail-actions"><a class="button" href="{{ url('/signup-option') }}">Book a consultation</a><a class="button button--light" href="{{ url('/services-v2') }}">View all services</a></div>
            </article>
        </div>
    </section>
@endsection
